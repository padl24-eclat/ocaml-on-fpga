  ...
  let step (s : state) : state =
   ...
   print_string "|env:";  print_val env;
    ...
    let rec branch_if (doInv) =
      ...
      else (pc_plus_2, acc, sp, other_regs, others) in
    ...
    | 82 (* GETSTRINGCHAR *) -> (* currently implement string has char array, 
                                 since char cannot be simply packed in long value 
                                 (having customized syze) *)
                                let (n,sp) = pop_stack(sp) in
                                let next_acc = get_field(acc,as_short(long_val(n))) in
                                (pc_plus_1,next_acc,sp, other_regs, others)
    | 83 (* SETSTRINGCHAR *) -> (* currently implement string has char array, 
                                 since char cannot be simply packed in long value 
                                 (having customized syze) *)
                                let (n,sp) = pop_stack(sp) in
                                let (v,sp) = pop_stack(sp) in
    ...
    | 143 (* STOP *) -> print_string "STOP : "; (pc, acc, sp, other_regs, caml_set_finish(others))
     | _ ->
      ...
      | 20 (* ASSIGN *) -> ram_set(sp_minus_1 - as_short(argument1), acc);       (* RAM_set ok ?  see ram.ml *)
...
(** [away(i,reset)] sustains value true as soon as
    input [i] is true until [reset] is false *)
let away (i,reset) : bool =
