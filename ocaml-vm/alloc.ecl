...
let is_pointer(start_space,v) =
  is_ptr v && start_space <= ptr_val v && ptr_val v < start_space + heap_size ;;
...
let gc_alloc ((sp, acc, env,sz) : (short * value *value * short)) : (value * value * short) = 
  ...
  let (next_acc,next_env,p,_,_,_) = 
    let step(next_acc,next_env,p, next, from_space, to_space) =
    ...
    in
    reg step last (acc, env,heap_start, heap_start, heap_start, heap_start + heap_size)
  in 
  (* print_string "~~~~========> "; print_int p; print_newline (); *)
  (next_acc,next_env,p) ;;
