...
let rec to_s ~statics g e x k =
  ...
  | E_par(e1,e2) ->
      ...
      let s1 = S_if(id1^"_started",S_skip,Some (seq_ (S_set(id1^"_started",A_const (Bool true))) @@ s1)) in
      let s2 = S_if(id2^"_started",S_skip,Some (seq_ (S_set(id2^"_started",A_const (Bool true))) @@ s2)) in
      
      SMap.empty ,SMap.empty, seq_ (S_fsm(id1,rdy1,res1,compute1,ts1,s1,false)) @@
                  seq_ (S_fsm(id2,rdy2,res2,compute2,ts2,s2,false)) @@
                  let_plug_s (A_call(Runtime And,A_tuple[A_var rdy1;A_var rdy2])) (fun z ->
                  S_if(z,
                      (seq_ (S_set(x,(A_tuple[A_var res1;A_var res2]))) @@
                       seq_ (S_set(id1^"_started",A_const (Bool false))) @@
                       seq_ (S_set(id2^"_started",A_const (Bool false))) k),None))
